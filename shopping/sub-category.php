...
$curSub = db_fetch_one("SELECT subcategory.subcategory, subcategory.categoryid, category.categoryName FROM subcategory LEFT JOIN category ON category.id=subcategory.categoryid WHERE subcategory.id=?", [$scid], "i");
$subName = $curSub['subcategory'] ?? 'Subcategory Products';
?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<?php 
$subDesc = "Browse authentic " . $subName . " in " . ($curSub['categoryName'] ?? 'our catalog') . ". Real-time multi-currency pricing and express shipping.";
render_seo_meta($subName . " - Hardware Catalog | ZeyTech", $subDesc, "sub-category.php?scid=" . $scid, "", "website");
?>
	...
	<div class='container'>
		...
		<div class="modern-hero-banner" style="margin-bottom:28px; padding:32px 36px;">
			<div class="hero-eyebrow">
				...
				[SUB-DOMAIN] &bull; <?php echo strtoupper(e($curSub['categoryName'] ?? 'CATEGORY')); ?>
			</div>
			<h1 class="hero-headline" style="font-size:28px; margin-bottom:8px;"><?php echo e($subName); ?></h1>
			<p class="hero-subtext" style="margin-bottom:0; font-size:13px;">Dedicated hardware inventory verified at Casablanca Hub-A1.</p>
		</div>
		...
		<div class='row outer-bottom-sm'>
			...
			<div class='col-md-3 sidebar'>
				...
				<div class="manifest-panel" style="padding:16px; margin-bottom:20px;">
					...
					<ul class="list-unstyled" style="margin:0;">
						<?php 
						$subcats = db_fetch_all("SELECT id, subcategory FROM subcategory ORDER BY subcategory ASC");
						foreach ($subcats as $sc) {
							$isActive = ($scid === intval($sc['id']));
						?>
						<li style="margin-bottom:4px;">
							<a href="sub-category.php?scid=<?php echo e($sc['id']);?>" style="display:flex; justify-content:space-between; align-items:center; padding:6px 8px; color:<?php echo $isActive ? '#d9b567' : '#8ea2bf'; ?>; background:<?php echo $isActive ? 'rgba(199,154,68,0.12)' : 'transparent'; ?>; font-family:'IBM Plex Sans'; font-size:13px; font-weight:<?php echo $isActive ? '700' : '400'; ?>; text-decoration:none; border-radius:2px;">
			...
			<div class='col-md-9'>
				<div class="row">
					<?php
					$subProducts = db_fetch_all("SELECT * FROM products WHERE subCategory=? ORDER BY id DESC", [$scid], "i");
					...
					<div class="col-xs-12 col-sm-6 col-md-4">
						<div class="modern-product-card">
							...
							<div class="product-category-label"><?php echo e($subName); ?></div>
							...
							<div class="product-price-row">
								...
								<a href="sub-category.php?scid=<?php echo e($scid);?>&action=add&id=<?php echo e($row['id']); ?>" class="btn-add-cart-modern">
									+ ADD
								</a>
								<?php } else { ?>
					...
					<div class="col-xs-12" style="text-align:center; padding:60px 20px; background:#0c1526; border-radius:2px; border:1px solid rgba(142,162,191,0.18);">
						<h3 style="font-family:'Fraunces', serif; color:#f2efe6; font-weight:700;">No Items In This Sub-Domain</h3>
						<p style="color:#8ea2bf; font-family:'Space Mono'; font-size:12px; margin-bottom:20px;">Check back shortly as new shipments are logged.</p>
